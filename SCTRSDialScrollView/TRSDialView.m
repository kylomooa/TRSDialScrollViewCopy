...
- (void)setDialRangeFrom:(CGFloat)from to:(CGFloat)to {
...
//    frame.size.width = (_maximum - _minimum) * _minorTickDistance + self.superview.frame.size.width;
    frame.size.width = (_maximum - _minimum) *10* _minorTickDistance + self.superview.frame.size.width;
...
- (void)drawTicksWithContext:(CGContextRef)context atX:(long)x
{
    ...
    if ([self isMajorTick:x]) {
        ...
        //
//        int value = (point.x - self.leading) / self.minorTickDistance / 10 + _minimum;
        int value = (point.x - self.leading) / self.minorTickDistance + _minimum;
        NSString *text = @"";
        if (self.RulerEnumType == RulerEnumValueBG) {
            
            text = [NSString stringWithFormat:@"%i", value/10];
        }else{
